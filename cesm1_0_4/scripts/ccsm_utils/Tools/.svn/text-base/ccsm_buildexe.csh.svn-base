...
if ($USE_MPISERIAL == "TRUE" && $USE_ESMF_LIB == "TRUE") then
  echo " "
  echo "  ERROR USE_MPISERIAL IS TRUE AND USE_ESMF_LIB IS TRUE"
  echo "        USE_MPISERIAL can only be used with an ESMF library built with mpiuni on"
